...
const cardRequests = JSON.parse(open('./card-info-success.json'));

export default function () {
    let apiUrl = 'http://heraeveriliondev.cloudapp.net/ilionservices4/YPF_YER_INTEGRACION/api/TarjetaInfo/';
    let method = 'GetTarjeta/';
    let params = {
        headers: { 'Content-Type': 'application/json' },
        responseType: 'text',
    };

    for (let cardReq of cardRequests) {
        let body = {
            "IdContacto":cardReq.IdContacto,
            "TipoIdentificacion":cardReq.TipoIdentificacion,
            "Identificacion":cardReq.Identificacion
        };
